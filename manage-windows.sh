#!/usr/bin/env bash
# ============================================================================
#  Al Hayat al-Ula Android · management console · WINDOWS
...
SRC_BOOK_DIR="${SRC_BOOK_DIR:-$HOME/koutoub/06-al-hayat-al-ula}"
KEYSTORE_DIR="${KEYSTORE_DIR:-$HOME/.keys}"
KEYSTORE_FILE="${KEYSTORE_FILE:-$KEYSTORE_DIR/wdiy-upload.keystore}"
APP_ID="${APP_ID:-org.workshopdiy.alhayatalula}"
AVD_NAME="${AVD_NAME:-al_hayat_al_ula_test}"
...
cmd_test() {
  ...
  local out="/tmp/al-hayat-al-ula-$(date +%Y%m%d-%H%M%S).png"
...
show_menu() {
  ...
  cat <<EOF
${BOLD}${MAGENTA}+==================================================================+
|   Al Hayat al-Ula Android . management console (Windows $SHELL_ENV)${RESET}${BOLD}${MAGENTA}
